import { ssrRenderAttrs, ssrInterpolate, ssrRenderAttr, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderComponent, ssrRenderList } from "vue/server-renderer";
...
const _sfc_main$3 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<header${ssrRenderAttrs(mergeProps({ class: "p-4 bg-gray-800 text-white flex justify-between items-center" }, _attrs))}><h1 class="text-xl font-bold">📊 Gestion de Contadores Caja las Condes</h1><p>Total: ${ssrInterpolate(suma.value)}</p></header>`);
    };
  }
};
...
const _sfc_main$2 = {
  ...
  setup(__props) {
    useStore();
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "contador-card" }, _attrs))}><span>${ssrInterpolate(__props.contador.nombre)} (${ssrInterpolate(__props.contador.valor)})</span><div class="flex gap-2"><button>➕</button><button>➖</button><button>🗑️</button></div></div>`);
    };
  }
};
...
//# sourceMappingURL=index-B6f24qoY.js.map
